/*============================================================================================================
 ...
 @Script: ssi.datasim.create.daily.usage.idle.perc.js
 @Author: Danny Carvajal
 @Version: 1.0.0 
 @Date: 5/10/2024
 @Notes: getHourlyUsage() is a bit different than what we see in ssi.datasim.create.daily.usage.js. 
 This script allows me to set focus minutes bits to active for only
 a certain percentage of total usage.
 
 We want 20 uers to have hours 10 to 15 with 120 to 180 mins per day at 10% idle
      - Change line 27 to .10
      - Change line 205 to: let hourlyData = getHourlyUsage(120, 180, 0, 23, targetDate);
      - Change line 951 to: rt = 60;

 We want 20 uers to have hours 10 to 15 with 30 to 180 mins per day at 90% idle, but we also want less
 runtime for 20 of the users: 
       - Change line 27 to .90
       - Change line 205 to: let hourlyData = getHourlyUsage(30, 180, 0, 23, targetDate);
       - Change line 951 to: rt = 0;
=============================================================================================================*/
      ...
      for (let n = 0; n < applications.length; n++) {
            ...
            for (let i = 0; i < apps.length; i++) {
                  ...
                  for (let day = 1; day <= days; day++) {
                        ...
                        if (lupUsrCnt == 0 && mupUsrCnt == 0 && hupUsrCnt == 0) lupUsrCnt = 1;
...
// Scenario 1: 10 to 15 hours = 120 to 180 mins per day at 10% idle
// Scenario 2: 30 mins to 15 hours = 30 to 180 mins per day at 90% idle
let hourlyData = getHourlyUsage(120, 180, 0, 23, targetDate);
                                    let useTime = hourlyData[0]; 
                                    let runTime = hourlyData[1];
                                    let focusMinutes = hourlyData[2];
                                    let minutesInUse = useTime.getSum();
let uptimeMinutes = 1440;
//let uptimeMinutes = minutesInUse + Math.round((minutesInUse * .10));
                                    ...
                                    usage.set("focus_minutes",    (focusMinutes != null) ? focusMinutes.getBytes() : null);
...
/* ----------------------------------------------------------------------------------------------------------------------------------
  ...
  ---------------------------------------------------------------------------------------------------------------------------------*/
let getHourlyUsage = function(min, max, startHour, endHour, targetDate) {
      
      let binaries = "";
      let focusMinsEmpty = true;
      let usageMins = this.rand(min, max, 0);
      let notInFocusMins = usageMins * this.cfg.IDLEPERCENT;
      let focusMinutes = usageMins - notInFocusMins;
      let focusCnt = 0;
      //let useTime = getRandomizedArray(usageMins, startHour, endHour);
let useTime = getRandomizedArray(usageMins, 8, 17); // Leave this here for non-idle hours
...
// Keep non-used hours with uptime
if (i < 8 || i > 17) {
      rt = 60;
      ut = 0;
      focusMinutes = 0;
      ...
      if (targetDate.getDay() != 6 && targetDate.getDay() != 0) {
            focusMinutes = usageMins - notInFocusMins;      
            rt = 60;
      } else {
            ut = 0;
            focusMinutes = 0;
            rt = 60;
                  ...
                  for (var y = 0; y < 8; y++) {
                              ...
                              for (let x = (y!=7) ? 0 : 4; x < 8; x++) {
                                    if (focusCnt < focusMinutes) {
                                          let bit = (minCnt > 0) ? 1 : 0;
                                          curBinary += bit;
                                          if (bit==1) focusCnt++;
                                    } else {
                                          curBinary += 0;
...
let getRandomizedArray = function(total, startHour, endHour) {
      ...
      for (i = 0; i < tempArray.length; i++) {
            ...
            sum = sum + total;
//let entity = this.mgr.create("dc_test");
//entity.set("value", "Hour " + i);
//entity.set("value2", ansa[i]);
//entity.save();                                  
