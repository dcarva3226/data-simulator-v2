/*============================================================================================================
 ...
 @Script: ssi.datasim.create.daily.usage.js
 @Author: Danny Carvajal
 @Version: 2.0.0 
 @Date: 3/27/2024 - Created
 @Date: 5/12/2025 - Added function to create Program Usage Details.
 @Notes: see individual functions below for more details.

 There are other scripts that can be run after this one in the order below:
 1) Hourly Usage creator script
 2) User Logon creator script
 3) User break creator script
=============================================================================================================*/
...
let tbl = {
      ...
      pud               : "cmdb_program_usage_details",
                  ...
                  for (let day = 1; day <= days; day++) {
                        ...
                        for (let x = 0; x < usrLen; x++) {
                              ...
                              if (objCfg.create_pdu) {
                                    ...
                                    if (this.rand(1, 10, 0) == 9) continue;
                                    ...
                                    let keystrokes = (minutesInUse > 0) ? this.rand(minKeys, maxKeys, 0) : 0;
                                    let mouseClicks = (minutesInUse > 0) ? this.rand(minMous, maxMous, 0) : 0;
                                    let thinClient = false;
                                    if (!isWebApp) programInstance = getOrCreateProgramInstance(installedOn, app);
                                    ...
                                    usage.set("day_of_week",      dayOfweek);
                                    usage.set("keystrokes",       keystrokes);
                                    usage.set("minutes_in_use",   minutesInUse);
                                    usage.set("uptime_minutes",   uptimeMinutes);
                                    usage.set("mouse_clicks",     mouseClicks);
                                    ...
                                    usage.set("thin_client",      thinClient);
...
// Create Program Usage Details
let createProgramUsageDetails = function(installedOn, dayOfWeek, minutesInUse, useTime, keystrokes, 
                                          programInstance, thinClient, usageDate, mouseClicks, user, app) {

      if (minutesInUse == 0) return null;
      let lastHourMins = 0;
      
      // Get the array of useTime, passed in, that is non-zero
      let nonZeroHours = [];      
      for (let x = 0; x < 24; x++) {
            let ut = useTime[x];
            if (ut != 0 && ut !== undefined) {
                  nonZeroHours.push([x, ut]);
            }
      }

      let keys = splitNumber(keystrokes, nonZeroHours.length);
      let clicks = splitNumber(mouseClicks, nonZeroHours.length);

      for (let i = 0; i < nonZeroHours.length; i++) {
            
            if (jobHandle.isStopped()) throw "Manual script cancellation...";            

            let hours = nonZeroHours[i];
            let startTime = buildDate(usageDate, hours[0], this.rand(0, 59, 0), this.rand(0, 59, 0));
            let endTime = new Date(startTime.getTime() + hours[1] * 60000);

            let diffInMs = Math.abs(new Date(startTime) - new Date(endTime)); // Get difference in milliseconds
            let secs = Math.floor(diffInMs / 1000); // Convert to seconds

            let pud = this.mgr.create(this.tbl.pud);
            pud.set("computer", installedOn);
            pud.set("day_of_week", dayOfWeek);
            pud.set("duration_seconds", secs);
            pud.set("start_time", startTime);
            pud.set("end_time", new java.util.Date(endTime));
            pud.set("keystrokes", keys[i]);
            pud.set("mouse_clicks", clicks[i]);
            pud.set("program_instance", programInstance);
            pud.set("thin_client", false);
            pud.set("usage_date", usageDate);
            pud.set("used_from", installedOn);
            pud.set("user", user);
            pud.set("webapp", (!programInstance) ? app : null);
            if (this.cfg.update) pud.save();
      }
};
...
let getHourlyUsage = function(min, max, startHour, endHour) {
      ...
      let useTime = splitNumberHourly(usageMinutes, startHour, endHour); // Get a randomized array
      //cfg.debug = usageMinutes + "===" + useTime;
      let runTime = [];
...
// Takes a number (n) and splits in two an array of x parts that adds up to n.
let splitNumber = function(target, parts) {
    
      let result = [];
      let remaining = target;

      for (let i = 0; i < parts - 1; i++) {
            let num = Math.floor(Math.random() * (remaining / (parts - i) * 2)); // Random split
            result.push(num);
            remaining -= num;
      }

      result.push(remaining); // Ensure total adds up to target
      return result;
};

let splitNumberHourly = function(target, startHour, endHour) {
                 
      let arr = []; // Initialize an empty array
      for (let i = 0; i < 24; i++) {
            arr[i] = 0; // Set all positions to zero initially
      }

      let remaining = target;
      for (let i = startHour; i < endHour - 1; i++) {  
            let max = remaining / (endHour - i); // Adjusted max range
            let value = Math.floor(Math.random() * (max * 1.5)); // Increase randomness
            arr[i] = value;
            remaining -= value;
      }

      arr[endHour-1] = remaining; // Assign the remaining value to the last selected index
